...
class MetaCorgiSnacks
  ...
  #   # I SHOULD HAVE USED STRING INTERPOLATION INSTEAD OF CONDITIONAL!!
  #   var = name.to_s
  #   if var == 'bone'
  ...
  def self.define_snack(name)
    define_method(name) do
      ...
      # is caps_name necessary?
      # why not just caps_name = name.capitalize
      caps_name = "#{name.split('_').map(&:capitalize).join(' ')}"
      result = "#{caps_name}: #{info}: #{tastiness}"
